...
func (h *UserHandler) GetJWT(w http.ResponseWriter, r *http.Request) {
	...
	expiresInt := r.Context().Value("JwtExpiresIn").(int)
	...
	dadosJWT := map[string]interface{}{
		"sub": u.ID.String(),
		"exp": time.Now().Add(time.Second * time.Duration(expiresInt)).Unix(),
	}

	_, token, _ := jwt.Encode(dadosJWT)
	accesToken := dto.GetJWTOutput{AcessToken: token}
